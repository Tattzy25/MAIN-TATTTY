...
export default function Page() {
  ...
  const handleSubmit = async () => {
    if (!prompt.trim())
      return setError("Please enter a tattoo prompt to continue.");
    ...
    try {
      ...
      if (response.success && response.output) {
        ...
        setError(
          response.error || "Failed to generate tattoo. Please try again.",
        );
      }
    } catch (err: any) {
      setError(
        `Generation error: ${err.message}. Check your connection and try again.`,
      );
    } finally {
    ...
    <UploadedImagesProvider>
      <div className={LAYOUT_STYLES.pageContainer}>
        ...
        <div className="shrink-0 px-4 sm:px-6 md:px-8 lg:px-10 text-center">
          <h1 className="headline text-4xl sm:text-6xl lg:text-8xl pt-3 sm:pt-4 md:pt-6 lg:pt-8">
            Screw Ups
          </h1>
          <p className="sub-headline text-lg sm:text-xl max-w-2xl mx-auto mt-4">
              ...
              <div className="space-y-6">
                ...
                <div className="text-center">
                  <Button
                    ...
                    size="lg"
                    className="px-8 h-16 sub-headline-bold"
                  >
                ...
                Array.isArray(result) ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-6xl mx-auto px-4">
                    {result.map((img, idx) => (
                      <div
                        key={idx}
                        className="relative rounded-2xl sm:rounded-3xl overflow-hidden border-8 sm:border-12 lg:border-16 border-muted shadow-2xl ring-1 sm:ring-2 ring-border"
                      >
